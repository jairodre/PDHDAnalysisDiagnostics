 ...
 *
 * This is a header-only schema placeholder: it currently declares no records.
 * Populate it only with stable, reusable records shared by more than one
 * implemented analyzer. Every record must document units, validity flags,
 * sentinel policy, and stable event/object keys. Keep reconstructed, truth,
 * calibrated, uncalibrated, SCE, and no-SCE quantities distinct. ROOT-only buffers
